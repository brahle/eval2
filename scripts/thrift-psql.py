...
def generate_conv(out, models):
    ...
    includes = ['string', 'pqxx/result']
    namespace = ['eval', 'tuna']

    out.write("/*\n * this file is generated by scripts/thrift-psql.py\n");
    out.write(" * don't change it unless you know what are you doing.\n");
    out.write(" * all changes will be rewriten on next thrift-pqsl call.\n\n");
    out.write(" */\n")

    out.write("".join(['#include <'+i+'>\n' for i in includes]))
    out.write("\n")

    out.write("using std::string;\n")
    out.write("using pqxx::result;\n")
    out.write("using boost::shared_ptr;\n\n")
    out.write("using boost::static_pointer_cast;\n\n")
    out.write("using namespace eval::model;\n")
    ...
    for model in models:
        ...
        out.write(tab*2 + "return static_pointer_cast<%s, void>(\n" % \
                    (cname)) 
